...
class Game:
    ...
    def __init__(self, screen):
        ...
        self.character_selection_1 = Character_Selection_Menu_1(self)
        self.character_selection_2 = Character_Selection_Menu_2(self)
        self.character_selection_3 = Character_Selection_Menu_3(self)
        self.character_selection_4 = Character_Selection_Menu_4(self)
        self.map_selection = Map_Selection_Menu(self)
        self.curr_menu = self.main_menu

        # manage sound
        self.sound_manager = SoundManager()
        self.Volume = Volume(self)
        self.Music = Volume(self)

        # Game mode
        self.mode_menu = Mode(self)   
        ...
        map_path = os.path.join(base_dir, '..', 'Map', 'map_cyber.tmx')
    ...
    def draw_attack_menu(self):
        ...
        pygame.draw.rect(self.screen, (255, 255, 255), (20, 530, 250, 150), 2)  # White border

        list_attacks = self.attaques
        # Draw each attack in the rectangle
        for i, attack in enumerate(list_attacks):
            color = (0, 255, 0) if i == self.selected_attack_index else (255, 255, 255)  # Highlight selected attack
            text = pygame.font.Font(None, 36).render(attack, True, color)
            self.screen.blit(text, (30, 550 + i * 30))  # Position attacks

    def teleportation_cells(self, screen):
        self.tp_cells = [(4, 6), (3, 16), (27, 11)]  # List of teleportation cells

        for player in self.player_units + self.enemy_units:  # Iterate through players and enemies
            for i in range(0, len(self.tp_cells)):  # Iterate through valid indices of the teleportation cells list
                if (player.x, player.y) == self.tp_cells[i]:
                    # Select an arrival cell different from the current cell
                    available_cells = [0, 1, 2]  # Teleportation cell indices
                    available_cells.remove(i)  # Remove the current cell index

                    # Choose an arrival cell from the remaining indices
                    r = random.choice(available_cells)
                    (self.arrival_x, self.arrival_y) = self.tp_cells[r]  # Select the arrival cell
                    player.x, player.y = (self.arrival_x + 1, self.arrival_y + 1)  # Update player coordinates
                    break  # Exit the loop as soon as a teleportation is performed

    def healing_cells(self, screen):
        self.heal_cells = [[23,10],[24,10],[24,11],[23,11],[7,17],[8,17],[7,18],[8,18]]
        color = WHITE
        color1 = (20, 255, 20)
        ...
        for i, player in enumerate(self.player_units):
            for j in range(0, len(self.heal_cells)):
                if player.x == self.heal_cells[j][0] and player.y == self.heal_cells[j][1]:
    ...
    def damage_cells(self, screen):
        self.dmg_cells = [[13,15],[14,15],[15,15],[16,15],[17,15],[18,15],[19,13],[20,13],[21,13],[22,13],[23,13],[24,13]]
        damage_amount = 5
        
        for i, player in enumerate(self.player_units):
            for j in range(0, len(self.dmg_cells)):
                if player.x == self.dmg_cells[j][0] and player.y == self.dmg_cells[j][1]:
        ...
        for i, enemy in enumerate(self.enemy_units):
            for j in range(0, len(self.dmg_cells)):
                if enemy.x == self.dmg_cells[j][0] and enemy.y == self.dmg_cells[j][1]:
                    ...
                    # Handle keyboard keys
                        ...
                        if self.menu_attaques:
                            ...
                            if event.key == pygame.K_DOWN:
                                self.selected_attack_index = (self.selected_attack_index + 1) % len(self.attaques)
                                selected_unit.update_red_case(self.attaques[self.selected_attack_index])
                                ...
                                self.menu_attaques = False
                                selected_attack = hero_selected.assign_attack_class(self.selected_attack_index)

                                for i, enemy in enumerate(self.enemy_units):
                                    enemy_selected = enemy.assign_character_class()
                                    enemy_health = self.list_enemy_health[i]
                                    new_enemy_health = selected_unit.attack(selected_attack, enemy_selected, enemy_health)
                        ...
                        if self.menu_attaques:
                            ...
                            if event.key == pygame.K_DOWN:
                                ...
                                for i, player in enumerate(self.player_units):
                                    ...
                                    player_health = self.list_player_health[i]
                                    new_player_health = selected_unit.attack(selected_attack, player_selected, player_health)
    ...
    def flip_display(self):
        ...
        heal_text = pygame.image.load('Map/Text_heal.png')
        heal_text = pygame.transform.scale(heal_text, (32*3, 32))
        self.screen.blit(heal_text, (32*26 + 10, 32*19 + 10))
        ...
        self.teleportation_cells(self.screen)
        # Draw healing cells
        self.healing_cells(self.screen)
        # Draw damage cells
        self.damage_cells(self.screen)
    ...
    def flip_display_end_screen(self):
        """Displays the end screen."""
        fond_fin = pygame.image.load('Menu/Wallpaper.png')
...
def main():
    ...
    player1 = Unit(game.character_selection_1.game_personnage, 15, 10, [32, 32], game)
    ...
    player2 = Unit(game.character_selection_2.game_personnage, 15, 12, [32, 32], game)
    ...
    player3 = Unit(game.character_selection_3.game_personnage, 22, 19, [32, 32], game)
    ...
    player4 = Unit(game.character_selection_4.game_personnage, 23, 19, [32, 32], game)
    ...
    # Main game loop
    while play and iteration < 100:
        game.handle_player_turn()
        if len(game.enemy_units) == 0:
            game.flip_display_end_screen()
