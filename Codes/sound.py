...
class SoundManager:
    def __init__(self):
        pygame.init()
        self.sounds = {
            # French keys kept for backward compatibility
            ...
            # English aliases used after translation
            'At_Your_Orders':        pygame.mixer.Sound("Sounds/A_vos_ordres.mp3"),
            'To_Battle':             pygame.mixer.Sound("Sounds/Au_Combat.mp3"),
            'Launch_Music':          pygame.mixer.Sound("Sounds/The_Avengers.mp3"),
            'Forward':               pygame.mixer.Sound("Sounds/En_Avant.mp3"),
            'Crush_Enemy':           pygame.mixer.Sound("Sounds/Ecrasons_ennemi.mp3"),
        }

    def bruit(self, name):
        if name in self.sounds:
            if name in ('Musique_lancement', 'Launch_Music'):
                self.sounds[name].play(-1)
            else:
                self.sounds[name].play()
        else:
            print(f"Sound '{name}' unreachable.")